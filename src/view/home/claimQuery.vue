<template>
  <div>
      <mt-header title="车险理赔">
        <router-link to="/" slot="left">
            <mt-button icon="back"></mt-button>
        </router-link>
      </mt-header>
     <a class="order-item" v-for="item in queryList" :key="item.id" @click="selectItem(item.contentUrl)" v-bind:href="'tel:'+item.remark2">
         <div>
             <img v-bind:src="item.imageUrl" alt="" class="logo">
             <span class="title">{{item.remark1}}</span>
         </div>
         <span>{{item.remark2}}</span>
     </a>
  </div>
</template>
<script>
